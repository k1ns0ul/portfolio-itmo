...
func Migrate(ctx context.Context, c *Client, dirFS fs.FS, dir string) error {
	...
	for _, name := range names {
		...
		if err := c.conn.Exec(ctx,
			"INSERT INTO anomalies.schema_migrations (name) VALUES (?)", name); err != nil {
...
func ensureRegistry(ctx context.Context, c *Client) error {
	const ddl = `CREATE DATABASE IF NOT EXISTS anomalies;
CREATE TABLE IF NOT EXISTS anomalies.schema_migrations (
    name String,
...
func appliedSet(ctx context.Context, c *Client) (map[string]bool, error) {
	rows, err := c.conn.Query(ctx, "SELECT name FROM anomalies.schema_migrations")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
