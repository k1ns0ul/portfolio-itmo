...
import (
	...
	"github.com/andrey/cfa-bonds/internal/coupon"
...
func main() {
	...
	if err := run(log, *once); err != nil {
		log.Error("coupon worker terminated", "err", err)
		os.Exit(1)
	}
}
...
func run(log *slog.Logger, once bool) error {
	...
	producer, err := kafka.NewProducer(cfg.Kafka.Brokers, cfg.Kafka.ClientID+"-coupon", log)
	if err != nil {
		log.Warn("kafka unavailable, events will not be published", "err", err)
	}
	if producer != nil {
		defer producer.Close()
	}

	svc := coupon.NewService(coupon.Deps{
		Pool:      pool,
	...
	if once {
		return svc.ProcessDueCoupons(ctx, time.Now())
	}

	ticker := time.NewTicker(cfg.Workers.TickInterval)
	defer ticker.Stop()
	log.Info("coupon worker started in daemon mode", "interval", cfg.Workers.TickInterval.String())
	if err := svc.ProcessDueCoupons(ctx, time.Now()); err != nil {
		log.Error("initial coupon pass failed", "err", err)
	}
	for {
		select {
		case <-ctx.Done():
			log.Info("coupon worker stopping")
			return nil
		case <-ticker.C:
			if err := svc.ProcessDueCoupons(ctx, time.Now()); err != nil {
				log.Error("coupon pass failed", "err", err)
			}
		}
	}
}
